...
/**
 * Runs on a machine. Periodically sends the master information about the worker. Contacts the execution servlet master
 * for executions to run and runs them. Settings can be changed from the servlet.
 * <p/>
...
public class ExecWorker implements Runnable {
    ...
    @Option
    public int numSuccessJobs, numJobs;
        ...
        this.runnerThread = new Thread(new Runnable() {
            public void run() {
                while (true) {
                    if (currJob != null) {
                        processJob(currJob);
                        currJob = null;
                    } else Utils.sleep(sleepInterval * 1000);
                }
            }
            ...
            public void processJob(Job job) {
                ...
                try {
                    ...
                    if (job.nice != 0) command = "nice -n " + job.nice + " " + command;
                    if (!StrUtils.isEmpty(job.workingDir)) command = Utils.makeRunCommandInDir(command, job.workingDir);
                    ...
                    if (success) numSuccessJobs++;
                    else logs("[WORKER] Job failed");
                } catch (Exception e) {
...
//    }
        ...
        while (true) {
            if (sendStatus()) {
                ...
                if (shouldGetJob()) currJob = getJob();
            }
    ...
    private String getProcSummary() {
        ...
        try {
            ...
            for (String line : Utils.systemGetStringOutput(cmd).split("\n")) {
                ...
                if (Utils.parseDoubleEasy(tokens[0]) > 50) heavyProcs.add(StrUtils.join(tokens, " "));
            }
    ...
    public boolean sendStatus() {
        ...
        // HACK: only first worker sends general machine information
        // so we don't print out this twice; should really be handled on the server
        // side
        request.put("cpuSpeed", "" + cpuSpeed);
        if (!workerName.endsWith("-2") &&
                !workerName.endsWith("-3") &&
                !workerName.endsWith("-4")) {
