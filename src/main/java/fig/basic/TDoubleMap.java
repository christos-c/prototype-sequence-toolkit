...
/**
 * Provides a map from objects to doubles. Motivation: provides a specialized data structure for mapping objects to
 * doubles which is both fast and space efficient. Feature 1: You can switch between two representations of the map: -
 * Sorted list (lookups involve binary search) - Hash table with linear probing (lookups involve hashing) Feature 2:
 * Sometimes, we want several maps with the same set of keys. If we lock the map, we can share the same keys between
 * several maps, which saves space.
 * <p/>
 * Note: in the sorted list, we first sort the keys by hash code, and then for equal hash code, we sort by the objects
 * values.  We hope that hash code collisions will be rare enough that we won't have to resort to comparing objects.
 * <p/>
...
public class TDoubleMap<T> extends AbstractTMap<T> implements Iterable<TDoubleMap<T>.Entry>, Serializable {
    ...
    public TDoubleMap(AbstractTMap<T> map) {
        ...
        if (map instanceof TDoubleMap) this.values = (double[]) ((TDoubleMap<T>) map).values.clone();
        else this.values = new double[keys.length];
    }
    ...
    public double getWithErrorMsg(T key, double defaultValue) {
        int i = find(key, false);
        if (i == -1) error("%s not in map, using %f", key, defaultValue);
        return i == -1 ? defaultValue : values[i];
    }
    ...
    private int getCapacity(int n, boolean compact) {
        int capacity;
        if (mapType == MapType.SORTED_LIST) capacity = compact ? n : n * growFactor;
        else if (mapType == MapType.HASH_TABLE) {
    ...
    /**
     * Return the first index i for which the target key is less than or equal to key i (00001111).  Should insert
     * target key at position i. If target is larger than all of the elements, return size().
     */
    private int binarySearch(T targetKey) {
        ...
        while (l < u) {
            ...
            if (targetHash < keyHash || (targetHash == keyHash && ((Comparable) targetKey)
                    .compareTo((Comparable) keys[m]) <= 0)) u = m;
            else l = m + 1;
        }
        return l;
    }
    ...
    private void repCheck() {
        ...
        if (mapType == MapType.SORTED_LIST) {
            ...
            for (int i = 1; i < num; i++) { // Make sure keys are sorted.
                ...
                int h2 = hash(keys[i]);
                assert h1 <= h2;
                if (h1 == h2) assert ((Comparable) keys[i - 1]).compareTo((Comparable) keys[i]) < 0;
            }
        }
    }
