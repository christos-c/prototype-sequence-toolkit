...
public class MapUtils {
    ...
    public static <S, T> void incr(Map<S, Map<T, Integer>> map, S key1, T key2, int dVal) {
        ...
        if (m == null) {
            ...
            m.put(key2, dVal);
        } else if (!m.containsKey(key2)) m.put(key2, dVal);
        else m.put(key2, m.get(key2) + dVal);
    }
    ...
    public static <S, T> List<T> getListMut(Map<S, List<T>> map, S key) {
        ...
        if (list == null) map.put(key, list = new ArrayList());
        return list;
    }
    ...
    public static <T> String topNToString(PriorityQueue<T> pq, int numTop) {
        ...
        if (pq.size() > numTop) sb.append(" ...(" + (pq.size() - numTop) + " more)");
        sb.append(" }");
        return sb.toString();
    }
