...
class OptInfo {
    ...
    public String getValueString() {
        ...
        if (o == null) return "";
        if (o instanceof ArrayList) return StrUtils.join((ArrayList) o);
    ...
    public String toString() {
        ...
        String s = String.format("%-30s <%5s> : %s [%s]", fullName(), typeStr(), gloss, valueStr);
        String t = getEnumStr();
        if (!t.equals("")) s += " " + t;
        return s;
    }
    ...
    public boolean set(List<String> l, boolean append) {
        try {
            ...
            if (!append) {
                ...
                if (isBool(field.getGenericType())) stringRepn = v.toString();
                else stringRepn = StrUtils.join(l);
//        field.set(obj, v);
                setField(v);
            } else {
                ...
                if (oldv instanceof ArrayList) ((ArrayList) oldv).addAll((ArrayList) v);
...
/**
 * Due to historical reasons, all the member functions are prefixed with do, and all the static functions (apply to the
 * global theParser instance) are not.
 * <p/>
 * 3/1/2007: static methods register and parse have been deprecated. Please create an instance and use the doRegister
 * and doParse counterparts.
 */
public class OptionsParser {
    ...
    public OptionsParser doRegister(String group, Object o) {
        if (objects.containsKey(group)) throw Exceptions.bad("Group name already exists: " + group);
        objects.put(group, o);
        ...
        for (Object o : objects) {
            if (o instanceof String) name = (String) o;
                ...
                if (name == null) {
                    ...
                    else name = o.getClass().getSimpleName();
                }
                doRegister(name, o);
                name = null;
            }
    ...
    private OptInfo matchOpt(ArrayList<OptInfo> options, String s) {
        ...
        if (completeMatches.size() + partialMatches.size() == 0) {
            if (!ignoreUnknownOpts) stderr.println("Unknown option: '" + s + "'; -help for usage");
            return null;
        }
        if (completeMatches.size() == 1) return completeMatches.get(0);
        if (completeMatches.size() == 0 && partialMatches.size() == 1) return partialMatches.get(0);
    ...
    private boolean readOptionsFile(ArrayList<OptInfo> options, String file) {
        if (new File(file).isDirectory()) file = new File(file, defaultDirFileName).toString();
        boolean ignoreOpts = new File(file).getName().equals(ignoreOptsFileName);
    ...
    public void doParseHard(String[] args) {
        if (!doParse(args)) throw new RuntimeException("Parsing '" + StrUtils.join(args) + "' failed");
    }
    ...
    public boolean doParse(String[] args) {
        ...
        for (int i = 0; i < args.length; ) {
            if (args[i].equals("-help")) { // Get usage help
                ...
                while (i < args.length) {
                    if (args[i].equals("--")) nextIsVerbatim = true;
                    else if (args[i].equals("---")) allIsVerbatim = !allIsVerbatim;
                    else {
                        if (!allIsVerbatim && !nextIsVerbatim && (isStrictPrefixOf(args[i], "+", "-", "++"))) break;
                        l.add(args[i]);
    ...
    private String isMissing(OptInfo o, List<OptInfo> optInfos) {
        ...
        if (!StrUtils.isEmpty(o.condReq)) {
            ...
            if (info == null) // Shouldn't happen, but if it does, the user will be notified
                ...
                if (value.equals(info.getValueString())) return o.toString() + ", " + o.condReq + " holds";
            }
        }
        return null;
    }
