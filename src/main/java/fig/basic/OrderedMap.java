...
/**
 * A container for mapping objects to objects. Keep track of the order of the objects (as they were inserted into the
 * data structure). No duplicate elements allowed.
 */
public class OrderedMap<S, T> {
    ...
    public void put(S key, T val) {
        ...
        if (map.containsKey(key)) keys.remove(key); // Remove last occurrence of key
        keys.add(key);
        map.put(key, val);
    }
    ...
    public boolean printEasy(String path) {
        return !StrUtils.isEmpty(path) && printEasy(new File(path));
    }
