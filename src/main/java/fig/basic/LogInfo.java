...
/**
 * The logging output has a tree structure, where each node is a line of output, and the depth of a node is its indent
 * level. A run is the sequence of children of some node. A subset of the lines in the run will get printed.
 */
public class LogInfo {
    ...
    /**
     * Invokes a method and wraps it's invocation in a track. The method must be public and not take any arguments.
     *
    ...
    public synchronized static void track(Object o, boolean printAllChildLines, boolean printIfParentPrinted) {
        if (indWithin()) {
    ...
    public synchronized static void logs(Object o) {
        if (forcePrint || (indWithin() && thisRun().newLine())) printLines(o);
    }
    ...
    private static void printLines(Object o) {
        ...
        if (s.indexOf('\n') == -1) print(s + "\n");
        else for (String t : StrUtils.split(s, "\n")) print(t + "\n");
    }
...
/**
 * A run is a sequence of lines of text, some of which are printed. Stores the state associated with a run.
 */
class LogRun {
    public LogRun() {
        watch = new StopWatch();
