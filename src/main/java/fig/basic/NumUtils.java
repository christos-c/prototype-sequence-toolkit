...
public class NumUtils {
    ...
    public static void assertIsFinite(double[] xs) {
        for (double x : xs) assert isFinite(x) : "Not finite: " + x;
    }
    ...
    // Return log(gamma(xx))
    private static double[] logGammaCoeff = {76.18009172947146, -86.50532032941677, 24.01409824083091,
    ...
    /**
     * Stolen from Radford Neal's fbm package. digamma(x) is defined as (d/dx) log Gamma(x).  It is computed here using
     * an asymptotic expansion when x>5.  For x<=5, the recurrence relation digamma(x) = digamma(x+1) - 1/x is used
     * repeatedly.  See Venables & Ripley, Modern Applied Statistics with S-Plus, pp. 151-152. COMPUTE THE DIGAMMA
     * FUNCTION.  Returns -inf if the argument is an integer less than or equal to zero.
     */
