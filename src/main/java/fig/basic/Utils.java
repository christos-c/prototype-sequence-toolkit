...
public class Utils {
    ...
    public static int setEqual(int oldValue, int newValue, int nullValue) {
        ...
        if (oldValue != newValue) throw Exceptions.bad("Mis-match: %d %d", oldValue, newValue);
        return newValue;
    }
    ...
    public static boolean createSymLink(String src, String dest) {
        try {
            ...
            try { return Runtime.getRuntime().exec(cmd).waitFor() == 0; } catch (InterruptedException e) {
                return false;
            }
        } catch (IOException e) {
            return false;
    ...
    public static boolean parseBooleanEasy(String s, boolean defaultValue) {
        if (s == null) return defaultValue;
        try { return Boolean.parseBoolean(s); } catch (NumberFormatException e) { return defaultValue; }
    }
    ...
    public static boolean parseBooleanHard(String s) {
        try { return Boolean.parseBoolean(s); } catch (NumberFormatException e) {
            throw new RuntimeException("Invalid format: " + s);
        }
    }
    ...
    public static Object parseEnum(Class c, String s) {
        ...
        for (Object o : c.getEnumConstants())
            if (o.toString().toLowerCase().equals(s)) return o;
        return null;
    }
    ...
    public static int parseTimeLength(String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            char c = s.charAt(i);
            if (Character.isDigit(c)) n = n * 10 + Integer.parseInt(c + "");
            else if (c == 'd') {
                sum += n * 60 * 60 * 24;
                n = 0;
            } else if (c == 'h') {
    ...
    public static int closeSystemEasy(String cmd, Process p) {
        try { return closeSystem(cmd, p); } catch (InterruptedException e) { return -1; }
    }

    public static void closeSystemHard(String cmd, Process p) {
        try {
            int status = closeSystem(cmd, p);
            if (status != 0) throw new RuntimeException("Failed: '" + cmd + "' returned status " + status);
    ...
    public static boolean system(String cmd, Writer finalOut, Writer finalErr)
        ...
        if (finalOut != null) IOUtils.copy(in, finalOut);
        in.close();
        BufferedReader err = CharEncUtils.getReader(p.getErrorStream());
        if (finalErr != null) IOUtils.copy(err, finalErr);
        err.close();
        return closeSystem(cmd, p) == 0;
    }

    public static boolean system(String cmd, OutputStream finalOut, OutputStream finalErr)
            throws IOException, InterruptedException {
        Process p = openSystem(cmd);
        p.getOutputStream().close();
    ...
    public static boolean systemLogin(String cmd, OutputStream finalOut, OutputStream finalErr)
            throws IOException, InterruptedException {
    ...
    public static void systemHard(String cmd, Writer finalOut, Writer finalErr) {
        try {
            if (!system(cmd, finalOut, finalErr)) throw new RuntimeException(cmd + " had non-zero exit status");
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    }
    ...
    public static boolean systemEasy(String cmd, Writer finalOut, Writer finalErr) {
        try { return system(cmd, finalOut, finalErr); } catch (Exception e) { return false; }
    }
    ...
    public static String systemGetStringOutputEasy(String cmd) {
        try { return systemGetStringOutput(cmd); } catch (Exception e) { return null; }
    }
