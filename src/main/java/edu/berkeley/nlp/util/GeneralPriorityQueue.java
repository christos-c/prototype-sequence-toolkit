 ...
 *
 * @author Dan Klein
 * @author Christopher Manning For each entry, uses ~ 24 (entry) + 16? (Map.Entry) + 4 (List entry) = 44 bytes?
 */
public class GeneralPriorityQueue<E> {

    /**
     * An <code>Entry</code> stores an object in the queue along with its current location (array position) and
     * priority. uses ~ 8 (self) + 4 (key ptr) + 4 (index) + 8 (priority) = 24 bytes?
     */
    ...
    /**
     * On the assumption that leftChild(entry) and rightChild(entry) satisfy the heap property, make sure that the heap
     * at entry satisfies this property by possibly percolating the element o downwards.  I've replaced the obvious
    ...
    /**
     * Searches for the object in the queue and returns it.  May be useful if you can create a new object that is
     * .equals() to an object in the queue but is not actually identical, or if you want to modify an object that is in
     * the queue.
     *
     * @return null if the object is not in the queue, otherwise returns the object.
     */
    ...
    public double getLastPriority() {
        return getLastEntry().priority;
    }

    /**
     * Promotes a key in the queue, adding it if it wasn't there already.  If the specified priority is worse than the
     * current priority, nothing happens.  Faster than add if you don't care about whether the key is new.
     *
    ...
    /**
     * Demotes a key in the queue, adding it if it wasn't there already.  If the specified priority is better than the
     * current priority, nothing happens.  If you decrease the priority on a non-present key, it will get added, but at
