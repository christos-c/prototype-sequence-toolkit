...
/**
 * Reusable iterator over the sentences in a document collection. The class is careful to not load all the data into
 * memory but instead will only queue a small number in memory. You can pass options to do things like tokenize and case
 * fold.
 *
 * @author aria42
 */
public class SentenceIterable implements Iterable<List<String>>, Runnable {
    ...
    public Iterator<List<String>> iterator() {
        List<File> files = IOUtils.getFilesUnder(dataRoot, prefix, extension, true);
        Iterator<List<String>> it = new MyIterator(files.iterator());
        Filter<List<String>> filter = new Filter<List<String>>() {
            public boolean accept(List<String> t) {
                return t.size() <= maxSentenceLength;
            }
        };
    ...
    public void run() {
        ...
        for (File file : files) {
            ...
            List<String> lines = Iterators.fillList(
                    new Iterators.TransformingIterator<List<String>, String>(it, new Method<List<String>, String>() {
