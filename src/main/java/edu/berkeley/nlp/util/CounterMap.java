...
public class CounterMap<K, V> implements java.io.Serializable {
    ...
    /**
     * Gets the sub-counter for the given key.  If there is none, a counter is created for that key, and installed in
     * the CounterMap.  You can, for example, add to the returned empty counter directly (though you shouldn't). This is
     * so whether the key is present or not, modifying the returned counter has the same effect (but don't do it).
     */
    ...
    public CounterMap(boolean identityHashMap) {
        this(identityHashMap ? new MapFactory.IdentityHashMapFactory<K, Counter<V>>() : new MapFactory.HashMapFactory<K, Counter<V>>(),
                identityHashMap ? new MapFactory.IdentityHashMapFactory<V, Double>() : new MapFactory.HashMapFactory<V, Double>());
    }
