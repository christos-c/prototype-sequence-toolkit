...
public class PrototypeSequenceModel implements PrototypeConstants, Serializable {
     ...
     */
    public void train(Iterable<List<String>> data, FeatureExtractor<String, String> wordFeatureExtractor, Map<String, Set<String>> labelToProtoMap) {
    ...
    private Counter<String> getWordFeatureCounter(String word) {
        return word.equals(START_WORD) || word.equals(STOP_WORD) ? new Counter<String>() : wordFeatureExtractor
                .extractFeatures(word);
    }
    ...
    private void extractFeatures() {
        ...
        LogInfo.logs("There are %d labels and %d predicates yielding %d features", labels.size(),
                featManager.getNumFeatures(), weightsManager.getNumWeights());
        cacheFeatures();
        LogInfo.end_track();
    }
    ...
    private void fillWordPotentials() {
        ...
        for (int w = 0; w < vocab.size(); ++w) {
            for (int l = 0; l < labels.size(); ++l) {
                ...
                assert !Double.isInfinite(potential) && potential >= 0.0 : String
                        .format("Bad potential: %.5f", potential);
    ...
    private double[][] getNodeLabelMarginals(double[][] nodeStateMarginals, int curLength) {
        ...
        for (int i = 0; i < curLength; ++i) {
            assert Math.abs(DoubleArrays.add(nodeStateMarginals[i]) - 1.0) < 1.0e-4 : String
                    .format("nodeMarginals[%d] = %.5f", i, DoubleArrays.add(nodeStateMarginals[i]));
            for (int s = 0; s < markovStates.size(); ++s) {
    ...
    public class Tagger {
        ...
        private StationaryForwardBackward taggingForwardBackwards = new StationaryForwardBackward(
                new PrototypeSequenceModelInternal());
