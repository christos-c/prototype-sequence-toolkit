...
public class PartOfSpeechTester implements Runnable {
    ...
    @Option(gloss = "Where to write PrototypeSequenceModel")
    ...
    private FeatureExtractor<String, String> getFeatureExtractor(Map<String, Set<String>> prototypeMap) {
    ...
    private Map<String, Set<String>> getPrototypeMap(final Iterable<Tree<String>> trees) {
    ...
    private Iterable<List<String>> getData(final Iterable<Tree<String>> trees) {
        return IterableAdapter.adapt(trees, new IterableAdapter.Convertor<Tree<String>, List<String>>() {
    ...
    private Iterable<Tree<String>> getTrees() {
        ...
        if (!new File(treebankPath).isDirectory()) {
            throw new RuntimeException(String.format("%s doesn't exist", treebankPath));
        }
        return fetcher.getTrees(treebankPath, startSection, stopSection);
    }
