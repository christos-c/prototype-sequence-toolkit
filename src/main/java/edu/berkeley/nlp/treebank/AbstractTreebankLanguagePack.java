 ...
 * Only the abstract methods below need to be implemented to give a reasonable solution for a new language.
 *
 * @author Christopher Manning
 * @version 1.1
 */
public abstract class AbstractTreebankLanguagePack implements TreebankLanguagePack, Serializable {
     ...
     * Traditionally, EVALB has ignored a subset of the total set of punctuation tags in the English Penn Treebank
     * (quotes and period, comma, colon, etc., but not brackets)
     *
    ...
    /**
     * Accepts a String that is a punctuation word, and rejects everything else. If one can't tell for sure (as for ' in
     * the Penn Treebank), it maks the best guess that it can.
     *
    ...
    /**
     * Accepts a String that is a punctuation tag that should be ignored by EVALB-style evaluation, and rejects
     * everything else. Traditionally, EVALB has ignored a subset of the total set of punctuation tags in the English
    ...
    /**
     * Returns a filter that accepts a String that is a punctuation word, and rejects everything else. If one can't tell
     * for sure (as for ' in the Penn Treebank), it makes the best guess that it can.
     *
     * @return The Filter
     */
    ...
    /**
     * Returns a filter that accepts a String that is not a punctuation word, and rejects punctuation. If one can't tell
     * for sure (as for ' in the Penn Treebank), it makes the best guess that it can.
     *
     * @return The Filter
     */
    ...
    /**
     * Returns a filter that accepts a String that is a punctuation tag that should be ignored by EVALB-style
     * evaluation, and rejects everything else. Traditionally, EVALB has ignored a subset of the total set of
     * punctuation tags in the English Penn Treebank (quotes and period, comma, colon, etc., but not brackets)
     *
     * @return The Filter
     */
    ...
    /**
     * Returns a filter that accepts everything except a String that is a punctuation tag that should be ignored by
     * EVALB-style evaluation. Traditionally, EVALB has ignored a subset of the total set of punctuation tags in the
     * English Penn Treebank (quotes and period, comma, colon, etc., but not brackets)
     *
     * @return The Filter
     */
    ...
    /**
     * Return the input Charset encoding for the Treebank. See documentation for the <code>Charset</code> class.
     *
    ...
    /**
     * Return an array of characters at which a String should be truncated to give the basic syntactic category of a
     * label. The idea here is that Penn treebank style labels follow a syntactic category with various functional and
     * crossreferencing information introduced by special characters (such as "NP-SBJ=1").  This would be truncated to
     * "NP" by the array containing '-' and "=".
     *
    ...
    /**
     * Returns the index of the first character that is after the basic label.  That is, if category is "NP-LGS", it
     * returns 2. This routine assumes category != null This routine returns 0 iff the String is of length 0. This
     * routine always returns a number &lt;= category.length(), and so it is safe to pass it as an argument to
     * category.substring().
     */
    ...
    /**
     * Returns the basic syntactic category of a String. This implementation basically truncates stuff after an
     * occurrence of one of the <code>labelAnnotationIntroducingCharacters()</code>. However, there is also special case
     * stuff to deal with labelAnnotationIntroducingCharacters in category labels: (i) if the first char is in this set,
     * it's never truncated (e.g., '-' or '=' as a token), and (ii) if it starts with one ofthis set, a second item of
     * this set is also excluded (to deal with '-LLB-', '-RCB-', etc.).
     *
    ...
    /**
     * Returns the syntactic category and 'function' of a String. This normally involves truncating numerical
     * coindexation showing coreference, etc.  By 'function', this means keeping, say, Penn Treebank functional tags or
     * ICE phrasal functions, perhaps returning them as <code>category-function</code>.
     * <p/>
     * This implementation strips numeric tags after label introducing characters (assuming that non-numeric things are
     * functional tags).
     *
     * @param category The whole String name of the label
    ...
    /**
     * Returns the index within this string of the last occurrence of a isLabelAnnotationIntroducingCharacter which is
     * followed by only digits, corresponding to a numeric tag at the end of the string. Example:
     * <code>lastIndexOfNumericTag("NP-TMP-1") returns 6</code>.
     */
    ...
    private final Filter punctTagStringAcceptFilter = Filters.collectionAcceptFilter(punctuationTags());

    private final Filter punctWordStringAcceptFilter = Filters.collectionAcceptFilter(punctuationWords());

    private final Filter sFPunctTagStringAcceptFilter = Filters.collectionAcceptFilter(sentenceFinalPunctuationTags());

    private final Filter eIPunctTagStringAcceptFilter = Filters.collectionAcceptFilter(evalBIgnoredPunctuationTags());

    private final Filter startSymbolAcceptFilter = Filters.collectionAcceptFilter(startSymbols());
