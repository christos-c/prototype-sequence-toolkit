 ...
 * @author Dan Klein
 */
public class FigLBFGSMinimizer implements GradientMinimizer, Serializable {
    ...
    public double[] minimize(DifferentiableFunction function, double[] initial, double tolerance, boolean printProgress) {
        FigBacktrackingLineSearcher lineSearcher = new FigBacktrackingLineSearcher();
        double[] guess = DoubleArrays.clone(initial);
        for (int iteration = 0; iteration < maxIterations; iteration++) {
            LogInfo.track("Iteration %d", iteration + 1);
            ...
            if (printProgress) {
                LogInfo.logs("[LBFGSMinimizer.minimize] Iteration ended with value %.6f\n", nextValue);
            }

            if (iteration >= minIterations && converged(value, nextValue, tolerance)) return nextGuess;
    ...
    /**
     * The arguments passed to callback are - double[] currentGuess - int iterationDone
     *
    ...
    public FigLBFGSMinimizer() {
    }

    public FigLBFGSMinimizer(int maxIterations) {
        this.maxIterations = maxIterations;
    }
