...
public final class SloppyMath {
    ...
    /**
     * Returns the greater of two <code>float</code> values. That is, the result is the argument closer to positive
     * infinity. If the arguments have the same value, the result is that same value. Does none of the special checks
     * for NaN or -0.0f that <code>Math.max</code> does.
     *
     * @param a an argument.
    ...
    /**
     * Returns the greater of two <code>double</code> values. That is, the result is the argument closer to positive
     * infinity. If the arguments have the same value, the result is that same value. Does none of the special checks
     * for NaN or -0.0f that <code>Math.max</code> does.
     *
     * @param a an argument.
    ...
    /**
     * Returns the smaller of two <code>float</code> values. That is, the result is the value closer to negative
     * infinity. If the arguments have the same value, the result is that same value. Does none of the special checks
     * for NaN or -0.0f that <code>Math.max</code> does.
     *
     * @param a an argument.
    ...
    /**
     * Returns the smaller of two <code>double</code> values. That is, the result is the value closer to negative
     * infinity. If the arguments have the same value, the result is that same value. Does none of the special checks
     * for NaN or -0.0f that <code>Math.max</code> does.
     *
     * @param a an argument.
    ...
    /**
     * exponentiation like we learned in grade school: multiply b by itself e times. Uses power of two trick. e must be
     * nonnegative!!! no checking!!!
     *
     * @param b base
     * @param e exponent
     * @return b^e
    ...
    /**
     * Find a hypergeometric distribution. This uses exact math, trying fairly hard to avoid numeric overflow by
     * interleaving multiplications and divisions. (To do: make it even better at avoiding overflow, by using loops that
     * will do either a multiple or divide based on the size of the intermediate result.)
     *
    ...
    /**
     * Find a one-tailed Fisher's exact probability. Chance of having seen this or a more extreme departure from what
     * you would have expected given independence. I.e., k >= the value passed in. Warning: this was done just for
     * collocations, where you are concerned with the case of k being larger than predicted. It doesn't correctly handle
     * other cases, such as k being smaller than expected.
     *
     * @param k The number of black balls drawn
     * @param n The total number of balls
     * @param r The number of black balls
    ...
    public static double exp(double logX) {
        ...
        if (Math.abs(logX) < 0.001) return 1 + logX;
     ...
     *
     * @param args Either none, and the log add rountines are tested, or the following 4 arguments: k (cell), n (total),
     *             r (row), m (col)
     */
    public static void main(String[] args) {
        if (args.length == 0) {
            System.err.println(
                    "Usage: java edu.stanford.nlp.math.SloppyMath " + "[-logAdd|-fishers k n r m|-bionomial r n p");
            ...
            while (x < Double.MAX_VALUE) {
                ...
                if (x < Double.MIN_VALUE * 1000000) x *= 4;
                else x *= 1.0001;
            }
            double avgError = sumError / numTrials;
            System.out.println("Avg error was: " + avgError);
