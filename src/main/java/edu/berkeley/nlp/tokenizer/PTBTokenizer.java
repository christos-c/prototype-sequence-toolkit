...
/**
 * Tokenizer implementation that conforms to the Penn Treebank tokenization conventions. This tokenizer is a Java
 * implementation of Professor Chris Manning's Flex tokenizer, pgtt-treebank.l.  It reads raw text and outputs tokens as
 * edu.stanford.nlp.trees.Words in the Penn treebank format. It can optionally return carriage returns as tokens.
 *
...
public class PTBTokenizer extends AbstractTokenizer {
    ...
    /**
     * Constructs a new PTBTokenizer that treats carriage returns as normal whitespace. No source is specified, so
    ...
    /**
     * Constructs a new PTBTokenizer that optionally returns carriage returns as their own token. CRs come back as Words
     * whose text is the value of <code>PTBLexer.cr</code>.
     */
    ...
    protected Object getNext() {
        ...
        try {
            ...
            while (!tokenizeCRs && PTBLexer.cr.equals((String) token)) token = lexer.next();
        } catch (Exception e) {
            nextToken = null;
        }
        return token;
    ...
    /**
     * Reads a file from the argument and prints its tokens one per line. This is mainly as a testing aid, but it can
     * also be quite useful standalone to turn a corpus into a one token per line file of tokens.
     * <p/>
     * Usage: <code>java edu.stanford.nlp.process.PTBTokenizer filename </code>
     *
    ...
    /**
     * Returns a presentable version of the given PTB-tokenized words. Pass in a List of Words or Strings, or a Document
     * and this method will join the words with spaces and call {@link #ptb2Text(String) } on the output. This method
     * will check if the elements in the list are subtypes of Word, and if so, it will take the word() values to prevent
     * additional text from creeping in (e.g., POS tags). Otherwise the toString value will be used.
     */
    public static String ptb2Text(List ptbWords) {
        for (int i = 0; i < ptbWords.size(); i++)
            if (ptbWords.get(i) instanceof String) ptbWords.set(i, ((String) ptbWords.get(i)));
    ...
    static class PTBTokenizerFactory implements TokenizerFactory {
        ...
        /**
         * Constructs a new PTBTokenizer that optionally returns carriage returns as their own token. CRs come back as
         * Words whose text is the value of <code>PTBLexer.cr</code>.
         */
