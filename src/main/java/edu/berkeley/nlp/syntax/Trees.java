...
public class Trees {
    ...
    public static class FunctionNodeStripper implements TreeTransformer<String> {
        ...
        public static String transformLabel(Tree<String> tree) {
            ...
            if (cutIndex3 > 0 && (cutIndex3 < cutIndex2 || cutIndex2 == -1)) cutIndex2 = cutIndex3;
            if (cutIndex2 > 0 && (cutIndex2 < cutIndex || cutIndex <= 0)) cutIndex = cutIndex2;
    ...
    public static class PennTreeReader implements Iterator<Tree<String>> {
        ...
        private Tree<String> readTree(boolean isRoot) throws IOException {
            ...
            if (!lowercase || children.size() > 0) {
                return isRoot ? new NamedTree<String>(label, children, currTreeName) : new Tree<String>(label,
                        children);
            } else {
                return isRoot ? new NamedTree<String>(label.toLowerCase().intern(), children,
                        currTreeName) : new Tree<String>(label.toLowerCase().intern(), children);
            }
        }
    ...
    public static class PennTreeRenderer {
        ...
        private static <L> void renderTree(Tree<L> tree, int indent, boolean parentLabelNull, boolean firstSibling, boolean leftSiblingPreTerminal, boolean topLevel, StringBuilder sb) {
            ...
            final boolean suppressIndent = (parentLabelNull || (firstSibling && tree
                    .isPreTerminal()) || (leftSiblingPreTerminal && tree.isPreTerminal() && (tree
                    .getLabel() == null || !tree.getLabel().toString().startsWith("CC"))));
            if (suppressIndent) {
                sb.append(' ');
            } else {
                if (!topLevel) {
        ...
        private static <L> void renderFlat(Tree<L> tree, StringBuilder sb) {
            ...
            sb.append('(');
            if (tree.getLabel() == null) sb.append("<null>");
            else sb.append(tree.getLabel().toString());
            sb.append(' ');
    ...
    public static <L> Tree<L> asTree(List<L> leaves, L root) {
        ...
        final List<Tree<L>> children = new ArrayList<Tree<L>>();
    ...
    private static <L> List<Tree<L>> spliceNodesHelper(Tree<L> tree, Filter<L> filter, boolean splice) {
        ...
        if (filter.accept(tree.getLabel())) return splice ? splicedChildren : new ArrayList<Tree<L>>();
