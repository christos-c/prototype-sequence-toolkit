...
public class TreeJPanel extends JPanel {
    ...
    WidthResult widthResult(Tree tree, FontMetrics fM) {
        ...
        for (int i = 0; i < tree.getChildren().size(); i++) {
            WidthResult subWR = widthResult((Tree) tree.getChildren().get(i), fM);
            if (i == 0) {
    ...
    double paintTree(Tree t, Point2D start, Graphics2D g2, FontMetrics fM) {
        ...
        for (int i = 0; i < t.getChildren().size(); i++) {
            Tree child = (Tree) t.getChildren().get(i);
    ...
    public static void main(String[] args) throws IOException {
        ...
        String ptbTreeString = StrUtils.join(args, " ");
        Tree tree = (new Trees.PennTreeReader(new StringReader(ptbTreeString))).next();
        tjp.setTree(tree);
...
//    g2.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 1.0f));
        ...
        ImageIO.write(bi, "png", new File("tree.png")); //save as png format DONE!
