    ...
    public TreePath<L> findPath(Tree<L> start, Tree<L> end) {
        ...
        if (start != end) {
            ...
            for (int i = startPath.size() - 1; i > rootIndex; i--) {
                transitions
                        .add(new TreePath.Transition<L>(startPath.get(i), startPath.get(i - 1), TreePath.Direction.UP));
            }
            ...
            if (rootIndex < startPath.size() - 1 && rootIndex < endPath.size() - 1) {
                ...
                transitions.add(new TreePath.Transition<L>(endPath.get(rootIndex), endPath.get(rootIndex + 1),
                        postRootDirection));
            }
            ...
            for (int i = rootIndex + 1; i < endPath.size() - 1; i++) {
                transitions
                        .add(new TreePath.Transition<L>(endPath.get(i), endPath.get(i + 1), TreePath.Direction.DOWN));
            }
        }
